...
**Confirming a prod trigger.** Every path asks, and none of them makes you type anything. `repipe run` and `repipe rerun` show a Yes/No menu with **No** pre-selected (or pass `--yes` for CI); the interactive flow uses its own **Confirm** step, so you're never asked to retype a name you just picked off a list. You still get the red `[prod]` badge, an explicit environment pick, and a printed summary before it fires.
